...
export async function GET(ctx: APIContext): Promise<Response> {
  ...
  // `/sitemap-content.xml`. An UNRECOGNISED host still falls through, same as the index route: the
  // request reached us somehow, and the platform's own answer is never wrong there.
  const host = ctx.request.headers.get('host') ?? '';
  if (host && !isPlatformHost(host) && (await getStoreByCustomDomain(host.toLowerCase().replace(/:\d+$/, '').trim()))) {
    return notFound();
  }
