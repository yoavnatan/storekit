...
export function initOrdersTab(onAlertsChanged: () => void): void {
  ...
  function bindOrderCard(card: HTMLElement): void {
    ...
    // A STICKY NOTE — a square with its bottom corner peeled — and not the file/page glyph it used
    // to be. The order card grew a second small grey icon beside this one (the invoice chip,
    // `order-invoice-row.ts`), and two document silhouettes at 13px are one icon as far as the eye
    // is concerned. The page glyph stays where it means a FILE and not a note: the reports tab, and
    // the two CSV buttons. Every place a private note is marked uses this shape — chip, list row,
    // the product row's chip and the note field's own label — so changing one means changing all.
    ...
    function renderNotes(): void {
      ...
      notes.forEach((n, i) => {
        ...
        li.innerHTML = `${NOTE_ICON}<span class="order-note-text flex-1 min-w-0 [color:var(--color-text)] whitespace-pre-wrap break-words"></span>`
          + `<span class="order-note-actions inline-flex items-center gap-1 shrink-0"><button type="button" class="order-note-edit inline-flex items-center justify-center w-5 h-5 rounded-[var(--radius-sm)] [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-primary)]" aria-label="${esc(editLabel)}">${EDIT_ICON}</button><button type="button" class="order-note-del-trigger inline-flex items-center justify-center w-5 h-5 rounded-[var(--radius-sm)] [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-danger)]" aria-label="${esc(delLabel)}">${TRASH_ICON}</button></span>`
          + `<span class="order-note-confirm inline-flex items-center gap-1 shrink-0" hidden><button type="button" class="order-note-del-no inline-flex items-center justify-center w-5 h-5 rounded-[var(--radius-sm)] [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-text)]" aria-label="${esc(cancelLabel)}">${X_ICON}</button><button type="button" class="order-note-del-yes inline-flex items-center justify-center w-5 h-5 rounded-[var(--radius-sm)] [color:var(--color-danger)] bg-transparent border-0 cursor-pointer transition-[filter] duration-100 hover:brightness-90" aria-label="${esc(delLabel)}">${CHECK_ICON}</button></span>`;
  ...
  // Two independent columns. 'return' is the second, and it exists because the status cannot
  // express it: a return in progress leaves the order at 'delivered' and it stays there (owner,
  // 2026-08-20). A third, 'payout', answered "where is the money on this order" and went with the
  // custodial model — the processor pays each seller on its own schedule, so there is no per-order
  // answer to give.
  const ORDER_FILTER_COLUMNS = ['status', 'return'];
  /**
  ...
  function buildOrderCard(o: {
    ...
    notes?: string[];
    // `/api/seller/orders` returns the whole order (`scopeOrder`), so these arrive with no
    // endpoint change — they were simply never named here.
  ...
  function renderEomItems(items: EomItem[]) {
    ...
    list.innerHTML = items.map((item) => `
      <div class="eom-item" data-pid="${escEom(item.productId)}" data-price-agorot="${escEom(String(item.priceAgorot))}" data-qty="${escEom(String(item.qty))}">
        ...
        <button type="button" class="eom-item__del" aria-label="${escEom(tt('orderEditDeleteItem'))}">
