    ...
    function renderNotes(): void {
      ...
      notes.forEach((n, i) => {
        ...
        li.innerHTML = `${NOTE_ICON}<span class="order-note-text flex-1 min-w-0 [color:var(--color-text)] whitespace-pre-wrap break-words"></span>`
          + `<span class="order-note-actions inline-flex items-center gap-1 shrink-0"><button type="button" class="order-note-edit inline-flex items-center justify-center w-5 h-5 rounded [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-primary)]" aria-label="${editLabel}" title="${editLabel}">${EDIT_ICON}</button><button type="button" class="order-note-del-trigger inline-flex items-center justify-center w-5 h-5 rounded [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-danger)]" aria-label="${delLabel}" title="${delLabel}">${TRASH_ICON}</button></span>`
          + `<span class="order-note-confirm inline-flex items-center gap-1 shrink-0" hidden><button type="button" class="order-note-del-no inline-flex items-center justify-center w-5 h-5 rounded [color:var(--color-muted)] bg-transparent border-0 cursor-pointer transition-colors duration-100 hover:[color:var(--color-text)]" aria-label="${cancelLabel}" title="${cancelLabel}">${X_ICON}</button><button type="button" class="order-note-del-yes inline-flex items-center justify-center w-5 h-5 rounded [color:var(--color-danger)] bg-transparent border-0 cursor-pointer transition-[filter] duration-100 hover:brightness-90" aria-label="${delLabel}" title="${delLabel}">${CHECK_ICON}</button></span>`;
