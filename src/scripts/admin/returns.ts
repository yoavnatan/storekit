  ...
  document.querySelectorAll<HTMLButtonElement>('[data-admin-return]').forEach((btn) => {
    ...
    btn.addEventListener('click', () => {
      ...
      window.dispatchEvent(new CustomEvent('confirm:open', {
        detail: {
          title: forBuyer
            ? 'לזכות את הקונה במלוא הסכום?'
            : 'לסגור את הבקשה לטובת המוכר?',
          body: forBuyer
            ? 'הסכום יירשם כזיכוי לקונה ויקוזז מהמוכר. אין דרך לבטל את זה מהמסך הזה.'
            : 'הקונה לא יזוכה, והבקשה נסגרת. אין דרך לבטל את זה מהמסך הזה.',
          okLabel: forBuyer ? 'זכה את הקונה' : 'לטובת המוכר',
          onConfirm: async () => {
            try {
