...
export function initStoreGlow(): void {
  // Three independent callers ask for this on the same page now — the store-card
  // shelf, the spotlight carousel and the store header — and each only knows
  // about itself. The pass below is document-wide, so every call after the first
  // would re-walk the same images for nothing; one module instance per page
  // makes this the whole guard.
  if (inited) return;
  inited = true;
