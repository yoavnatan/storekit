...
// compatibility. The logo IS an image (public/logo-email.png, regenerated by
// scripts/generate-brand-assets.mjs): SVG doesn't render in mail, so the choice
// is a raster or nothing. It carries the brand name as its `alt`, so a client
// that blocks images by default — most of them, on first contact — still shows
// who the mail is from, in the header bar's own colour. Content builders
// (order-emails.ts) fill `bodyHtml`.
...
const COLORS = {
  ...
  muted: '#5a6478',
  border: '#e2e5eb',
  /* The brand surface — same value the site's buttons start from. The header bar
     and public/logo-email.png share it, so the image's plate is invisible. */
  brand: '#2a3c40',
} as const;
