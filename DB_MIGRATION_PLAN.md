...
**מה שמאמת את זה בלי להתקין כלום:** `tests/db-import.test.ts` מריץ את המיגרציה ואת הייבוא על
הקבצים האמיתיים מול Postgres בתוך התהליך (PGlite — Postgres שקומפל ל-WASM, אותם אילוצים ממש),
בתוך `npm test`. **מה שזה לא מכסה:** §9.2 (`EXPLAIN ANALYZE`) ו-§9.5 (50 רכישות מקבילות) —
