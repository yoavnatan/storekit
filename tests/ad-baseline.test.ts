...
describe('storeBaselineStatus', () => {
  it('carries a store that has something to promote', () => {
    const s = storeBaselineStatus({ storeId: 's1', blocked: false, visibleProductCount: 3 }, TODAY);
    expect(s.active).toBe(true);
    expect(s.impressions).toBeGreaterThan(0);
  });

  it('does not claim to advertise a store with no visible product', () => {
    expect(storeBaselineStatus({ storeId: 's1', blocked: false, visibleProductCount: 0 }, TODAY))
      .toEqual({ active: false, impressions: 0 });
  });

  it('does not claim to advertise a blocked store', () => {
    expect(storeBaselineStatus({ storeId: 's1', blocked: true, visibleProductCount: 9 }, TODAY))
      .toEqual({ active: false, impressions: 0 });
  });
  ...
  it('reports a real 30-day window, and is stable for the same store and day', () => {
    const a = storeBaselineStatus({ storeId: 's1', blocked: false, visibleProductCount: 1 }, TODAY);
    const b = storeBaselineStatus({ storeId: 's1', blocked: false, visibleProductCount: 1 }, TODAY);
    const other = storeBaselineStatus({ storeId: 's2', blocked: false, visibleProductCount: 1 }, TODAY);
