...
async function order(fields: {
  ...
  await query(
    `INSERT INTO orders (id, buyer_name, buyer_email, buyer_phone, buyer_city, buyer_street,
                         ...
                         paid_at, delivered_at, review_invited_at, created_at, updated_at)
     VALUES ($1, 'דנה', 'clock@example.test', '0500000000', 'תל אביב', 'הרצל 1',
             0, 1000, 'paid', $2, $3, $4, $5, $6, $6)`,
    [
      id,
      fields.shippingStatus ?? 'delivered',
      fields.paidAt === undefined ? daysAgo(30) : fields.paidAt,
...
describe('the payment fallback, for an order nobody marked delivered', () => {
  it('asks ten days after payment when there is no delivery stamp', async () => {
  ...
  it('waits while the fallback has not elapsed', async () => {
    const young = await order({ shippingStatus: 'shipped', deliveredAt: null, paidAt: daysAgo(3) });
    await runReviewInvites(NOW);
    expect(await invitedAt(young)).toBeNull();
  });

  it('does not let the fallback govern an order we KNOW arrived', async () => {
    // Paid long ago, delivered this morning. A single COALESCE'd clock would have fired on the
    // payment date and asked about a parcel the buyer opened an hour earlier.
    const justArrived = await order({ paidAt: daysAgo(40), deliveredAt: daysAgo(0) });
    await runReviewInvites(NOW);
...
describe('what it refuses to ask about at all', () => {
  it('skips an order that never shipped, and a cancelled one', async () => {
    const pending = await order({ shippingStatus: 'pending', deliveredAt: null, paidAt: daysAgo(60) });
