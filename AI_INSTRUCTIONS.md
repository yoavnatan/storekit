...
- **"Tactile depth" design language** (piloted on the product page, expand to other pages going forward) — elevated surfaces (image wraps, detail cards, secondary-item cards) get `border:1.5px solid var(--color-border)` + `box-shadow:var(--shadow-card)`, lifting to `--shadow-card-hover` on hover; recessed controls (qty steppers) get `--shadow-inset`; pressed/active chips get `--shadow-xs`. Tokens defined in `tokens.css`. Restrained on purpose — not neumorphism, not brutalist neon borders. Reference implementation: `src/styles/pages/product.css`.
