...
- **Orders — buyer dashboard:** "הזמנות" tab in buyer area — filtered by `o.buyerId === userId || o.buyerEmail === seller.email` (covers both linked and guest orders). Accordion cards with: order ID, date, items summary, payment/shipping badges, total in header; expanded body shows per-store product list with images + skeleton shimmer (`img.decode()` on open), store subtotal, tracking number if set.
