...
- **Store page `/store/[slug]`:** Product grid (no "מוצרים" heading); **product detail modal** (click image/name → modal; `history.pushState` syncs URL to `/store/slug/product`, `document.title` updates, `popstate` closes modal on browser back; ESC/backdrop close; image gallery with lightbox-from-modal; qty + add-to-cart + wishlist; direct URL → full SSR product page for SEO/sharing); lightbox (arrows/keyboard/swipe/touch); search + sort dropdown (search matches name + tags); **category filter chips** (derived from product `category` fields — no store config needed); **URL sync** (`history.replaceState` on every filter/sort/search change; `?q=`, `?category=`, `?sort=` params restored on page load; shareable/bookmarkable filtered URLs); add to cart (spring animation + qty stepper); wishlist hearts; "New" badge; `dir="auto"` bidi; **light banner** + **thin dark strip**. Backup pre-modal at `_backup/store-slug.astro`
